local isDead = false
RegisterNetEvent('meteo-deathscreen:openUI', function(time)
    isDead = true
...
RegisterNetEvent('meteo-deathscreen:revive', function()
    isDead = false
...
RegisterNetEvent('meteo-deathscreen:die', function(time)
    isDead = true
    SendNUIMessage({action = "DSMenu", open = true, time = time})
    if Config.Blur then
        TriggerScreenblurFadeIn()
...
RegisterNetEvent('meteo-deathscreen:res', function()
    SendNUIMessage({action = "res"})
end)

RegisterNetEvent('meteo-deathscreen:updateRes', function(time)
...
Citizen.CreateThread(function()
    local resourceName = "^3 This script was made by the QBHub team join here!, discord.gg/qbhub ("..GetCurrentResourceName()..")"
    print("\n^7----------------------------------------------------------------------------------^7")
    print(resourceName)
    print("^7----------------------------------------------------------------------------------^7")
end)